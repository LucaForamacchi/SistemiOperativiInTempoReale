...
int itemCompare(ItemType item1, ItemType item2) {
    if (strcmp(item1.player.name, item2.player.name) == 0 &&
        strcmp(item1.player.surname, item2.player.surname) == 0) {
        return 1;
    } else {
        return 0;
    }
...
LIST EnqueueOrdered(LIST l, ItemType item, int index) {
    LIST tmp = l;
    int i = 1;
    if(tmp == NULL){
        return NULL;
